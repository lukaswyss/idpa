...
export async function createSession(userId: string, days = 30): Promise<void> {
  ...
  await (prisma as any).authSession.create({ data: { userId, token, expiresAt } });
...
export async function getSessionUser(): Promise<{ id: string; username: string | null } | null> {
  ...
  const session = await (prisma as any).authSession.findUnique({ where: { token }, include: { user: true } });
